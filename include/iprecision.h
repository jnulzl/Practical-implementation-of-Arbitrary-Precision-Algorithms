...
static char _VI_[] = "@(#)iprecision.h 03.21 -- Copyright (C) Henrik Vestermark";
...
class int_precision
	...
	//class chacha20;
	//class chacha20(const std::vector<uint8_t>&, const std::vector<uint8_t>&, uint32_t);
...
public:
	...
	int_precision operator()(const std::uintmax_t bitcnt = 64)
	{
		...
		uniform_int_distribution<std::uintmax_t> disbits(0, bitcnt);
		uniform_int_distribution<std::uintmax_t> dis(0, (~0ull));  // Full 64bit range
		...
		if (a != 0)
		{
			...
			else
			{
				...
				uniform_int_distribution<std::uintmax_t> distop(0, a);      // Most significant range
				a = distop(generator);
			}
...
#if _MSC_VER
	xoshiro256pp(const std::seed_seq& seeds)
#else
    xoshiro256pp(std::seed_seq& seeds)
#endif
	{   // Initialization through seed_seq seed
		seed(seeds);
	}
...
#if _MSC_VER
	inline xoshiro256ss(const std::seed_seq& seeds)
#else
    inline xoshiro256ss(std::seed_seq& seeds)
#endif
	{   // Initialization through seed_seq seed
		seed(seeds);
	}
...
#if _MSC_VER
	inline void seed(const std::seed_seq& seeds)
#else
    inline void seed(std::seed_seq& seeds)
#endif
	{   // Initialization through seed_seq seed
		std::array<unsigned, 4> sequence;
		seeds.generate(sequence.begin(), sequence.end());
...
#if _MSC_VER
	inline xoshiro512pp(const std::seed_seq& seeds)
#else
    inline xoshiro512pp(std::seed_seq& seeds)
#endif
	{   // Initialization through seed_seq seed
		seed(seeds);
	}
...
#if _MSC_VER
	inline xoshiro512ss(const std::seed_seq& seeds)
#else
    inline xoshiro512ss(std::seed_seq& seeds)
#endif
	{   // Initialization through seed_seq seed
		seed(seeds);
	}
