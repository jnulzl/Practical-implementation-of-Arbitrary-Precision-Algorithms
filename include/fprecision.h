...
static char _VF_[] = "@(#)fprecision.h 03.25 -- Copyright (C) Henrik Vestermark";
...
// Enable usaing float_precision for int_precision division when appropriate
// This is enable or disbale in iprecision.h
...
inline float_precision float_precision::succ()
{
	...
	{
		next = one;
		next.exponent(-LLONG_MIN);  // if eptype==intmax_t
		return next;
	}
	one.exponent(-pbits);
	next += one;
